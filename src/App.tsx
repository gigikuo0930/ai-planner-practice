...
function Feedback({ question, correct }: { question: Question; correct: boolean }) { const note = getLearningNote(question); return <aside className={correct ? 'feedback good' : 'feedback bad'}><strong>{correct ? '答對了！' : `正確答案：${question.correct_answer}`}</strong><strong>重點判讀：為何選 {question.correct_answer}</strong><p>{note.answerReasoning}</p>{note.optionAnalysis.length > 0 && <><strong>選項比較</strong><ul className="option-analysis">{note.optionAnalysis.map((item) => <li key={item}>{item}</li>)}</ul></>}<strong>教材重點摘要</strong><p>{note.materialSummary}</p><strong>概念補充</strong><p>{note.conceptSummary}</p><strong>答案定位</strong><p>{note.locatingText}</p><p>學習指引：{question.guide_location.guide_heading}（{question.guide_location.guide_section}，紙本 {question.guide_location.guide_printed_pages}／PDF {question.guide_location.guide_pdf_pages}）</p>{question.version_sensitive && <p>⚠ 依原考次資訊：{question.exam_date ?? '日期未提供'}</p>}<small>{learningNoteSourceLabel(question, note)}</small></aside> }
