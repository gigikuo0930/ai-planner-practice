...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    {page === 'history' && <section className="page"><h2>學習紀錄</h2><p className="muted">點選一筆紀錄即可查看該次的逐題結果與完整題目。</p><RecordList records={data.records} detailed onSelect={setSelectedRecordId} />{(data.records.find((record) => record.id === selectedRecordId) ?? data.records[0]) && <ReviewResults record={data.records.find((record) => record.id === selectedRecordId) ?? data.records[0]} />}<h2>科目與主題表現</h2><CategoryStats records={data.records} /></section>}
...
function Feedback({ question, correct }: { question: Question; correct: boolean }) { const note = getLearningNote(question); return <aside className={correct ? 'feedback good' : 'feedback bad'}><strong>{correct ? '答對了！' : `正確答案：${question.correct_answer}`}</strong><p>{note.explanation}</p><strong>核心觀念</strong><p>{note.conceptSummary}</p><strong>答案定位</strong><p>{note.locatingText}</p><p>學習指引：{question.guide_location.guide_heading}（{question.guide_location.guide_section}，紙本 {question.guide_location.guide_printed_pages}／PDF {question.guide_location.guide_pdf_pages}）</p>{question.version_sensitive && <p>⚠ 依原考次資訊：{question.exam_date ?? '日期未提供'}</p>}<small>{learningNoteSourceLabel(question, note)}</small></aside> }
...
function QuestionStatusList({ attempts, onSelect }: { attempts: Attempt[]; onSelect?: (id: string) => void }) { const ids = [...new Set(attempts.filter((attempt) => !attempt.correct).map((attempt) => attempt.questionId))]; return ids.length ? <div className="status-list">{ids.map((id) => { const question = questions.find((item) => item.id === id)!; const status = getMastery(id, attempts); const retries = attempts.filter((attempt) => attempt.questionId === id && attempt.correct && attempt.countsForMastery).length; return <button className="question-row" key={id} onClick={() => onSelect?.(id)}><div><strong>{question.topic}</strong><span>{question.stem}</span></div><b className={status}>{masteryLabel[status]}</b><small>有效複習答對 {retries}/2 次 · 點選查看答案位置</small></button> })}</div> : <p className="empty">還沒有錯題紀錄。</p> }
...
function QuestionDetail({ question, selected }: { question: Question; selected?: Answer | null }) { const note = getLearningNote(question); return <article className="question-detail"><p className="meta">{question.subject} · {question.topic} · {sourceLabel[question.source_type]}</p><h3>{question.stem}</h3><div className="detail-options">{(['A', 'B', 'C', 'D'] as Answer[]).map((choice) => <p key={choice} className={choice === question.correct_answer ? 'correct-answer' : choice === selected ? 'your-answer' : ''}><b>{choice}</b>{question.options[choice]}{choice === selected && <small>你的答案</small>}{choice === question.correct_answer && <small>正確答案</small>}</p>)}</div><div className="answer-location"><strong>解析</strong><p>{note.explanation}</p><small>{learningNoteSourceLabel(question, note)}</small><strong>核心觀念摘要</strong><p>{note.conceptSummary}</p><strong>答案在哪裡找到？</strong><p>{note.locatingText}</p><p>學習指引：{question.guide_location.guide_heading}（{question.guide_location.guide_section}）</p><p>紙本頁碼：{question.guide_location.guide_printed_pages}　PDF 頁碼：{question.guide_location.guide_pdf_pages}</p>{note.caution && <p className="caution">⚠ {note.caution}</p>}</div></article> }
function ReviewResults({ record }: { record: RecordItem }) { const [selectedId, setSelectedId] = useState<string | null>(null); const selected = record.results.find((result) => result.questionId === selectedId); const question = selected ? questions.find((item) => item.id === selected.questionId) : undefined; return <section><h2>本次逐題結果</h2><div className="status-list">{record.results.map((result, index) => { const item = questions.find((question) => question.id === result.questionId)!; return <button className="question-row" key={result.questionId} onClick={() => setSelectedId(result.questionId)}><div><strong>第 {index + 1} 題 · {result.correct ? '答對' : '答錯'}</strong><span>{item.stem}</span></div><b>{result.selected ?? '未作答'}／{item.correct_answer}</b><small>點選查看完整題目與答案位置</small></button> })}</div>{question && <QuestionDetail question={question} selected={selected?.selected} />}</section> }
function WrongPage({ attempts, counts, onStart }: { attempts: Attempt[]; counts: { due: number; reviewed: number; mastered: number }; onStart: () => void }) { const [filter, setFilter] = useState<'due' | 'reviewed' | 'mastered'>('due'); const [selectedId, setSelectedId] = useState<string | null>(null); const visible = attempts.filter((attempt) => !attempt.correct).filter((attempt, index, all) => all.findIndex((item) => item.questionId === attempt.questionId) === index).filter((attempt) => getMastery(attempt.questionId, attempts) === filter); const question = selectedId ? questions.find((item) => item.id === selectedId) : undefined; const lastAttempt = selectedId ? attempts.filter((attempt) => attempt.questionId === selectedId).at(-1) : undefined; return <section className="page"><h2>錯題本</h2><p className="muted">錯題需在不同日累積兩次複習答對，才會標示為已掌握。</p><div className="status-grid"><StatusCard label="待複習" value={counts.due} state="due" /><StatusCard label="複習答對" value={counts.reviewed} state="reviewed" /><StatusCard label="已掌握" value={counts.mastered} state="mastered" /></div><div className="tab-row">{(['due', 'reviewed', 'mastered'] as const).map((state) => <button className={filter === state ? 'primary' : 'secondary'} key={state} onClick={() => { setFilter(state); setSelectedId(null) }}>{masteryLabel[state]}</button>)}</div><button className="primary wide" disabled={!counts.due} onClick={onStart}>重練待複習錯題</button><QuestionStatusList attempts={visible.flatMap((item) => attempts.filter((attempt) => attempt.questionId === item.questionId))} onSelect={setSelectedId} />{question && <QuestionDetail question={question} selected={lastAttempt?.selected} />}</section> }
