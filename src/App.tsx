...
function ReviewResults({ record }: { record: RecordItem }) { const [selectedId, setSelectedId] = useState<string | null>(null); return <section><h2>本次逐題結果</h2><div className="status-list">{record.results.map((result, index) => { const item = questions.find((question) => question.id === result.questionId)!; const expanded = selectedId === result.questionId; return <div className="question-result" key={result.questionId}><button className={`question-row ${expanded ? 'expanded' : ''}`} aria-expanded={expanded} onClick={() => setSelectedId(expanded ? null : result.questionId)}><div><strong>第 {index + 1} 題 · {result.correct ? '答對' : '答錯'}</strong><span>{item.stem}</span></div><b>{result.selected ?? '未作答'}／{item.correct_answer}</b><small>{expanded ? '收合完整題目與解析' : '點選查看完整題目與解析'}</small></button>{expanded && <QuestionDetail question={item} selected={result.selected} />}</div> })}</div></section> }
function WrongPage({ attempts, counts, onStart }: { attempts: Attempt[]; counts: { due: number; reviewed: number; mastered: number }; onStart: () => void }) { const [filter, setFilter] = useState<'due' | 'reviewed' | 'mastered'>('due'); const [selectedId, setSelectedId] = useState<string | null>(null); const visible = attempts.filter((attempt) => !attempt.correct).filter((attempt, index, all) => all.findIndex((item) => item.questionId === attempt.questionId) === index).filter((attempt) => getMastery(attempt.questionId, attempts) === filter); const visibleAttempts = visible.flatMap((item) => attempts.filter((attempt) => attempt.questionId === item.questionId)); return <section className="page"><h2>錯題本</h2><p className="muted">錯題需在不同日累積兩次複習答對，才會標示為已掌握。</p><div className="status-grid"><StatusCard label="待複習" value={counts.due} state="due" /><StatusCard label="複習答對" value={counts.reviewed} state="reviewed" /><StatusCard label="已掌握" value={counts.mastered} state="mastered" /></div><div className="tab-row">{(['due', 'reviewed', 'mastered'] as const).map((state) => <button className={filter === state ? 'primary' : 'secondary'} key={state} onClick={() => { setFilter(state); setSelectedId(null) }}>{masteryLabel[state]}</button>)}</div><button className="primary wide" disabled={!counts.due} onClick={onStart}>重練待複習錯題</button><QuestionStatusList attempts={visibleAttempts} expandedId={selectedId} onToggle={(id) => setSelectedId(id === selectedId ? null : id)} /></section> }
